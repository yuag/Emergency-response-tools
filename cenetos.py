...
class YaraScanner:
    ...
    def execute_all_commands(self, output_file_path="centos.html"):
        results = []
        ...
        commands = [
            ...
            ("监控与目标IP通信的进程","ps -ef"),
            ("CPU降序排序","top -n 1 -b"),
            ("查询历史命令","cat ~/.bash_history"),
            ("7天内被修改过的文件","find /usr/bin/ /usr/sbin/ /bin/ /usr/local/bin/ -type f -mtime +7 | xargs ls -la"),
            ("登录记录","last"),


            
            ("多少IP在爆破主机的root帐号","sudo grep Failed password for root /var/log/secure | awk '{print $11}' | sort | uniq -c | sort -nr | more"),
           
           
            
            ("哪些IP在爆破","sudo grep 'Failed password' /var/log/secure | grep -E -o '(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)' | uniq -c"),
            ("爆破用户名字典是什么", """sudo grep 'Failed password' /var/log/secure | perl -e 'while($_=<>){ /for(.*?) from/; print "$1\\n";}' | uniq -c | sort -nr"""),
    
            ("登录成功的日期、用户名、IP：", "sudo grep 'sshd.*Accepted' /var/log/secure | awk '{print $1,$2,$3,$9,$11}'"),
    ...
    def main(self):
        while True:
            ...
            if choice == '0':
                ...
                output, _ = self.execute_command("cat ~/.bash_history")
                print(output)
                ...
                output, _ = self.execute_command("sudo grep Failed password for root /var/log/secure | awk '{print $11}' | sort | uniq -c | sort -nr | more")
                print(output)

            elif choice == '12':
                output, _ = self.execute_command("sudo grep 'Failed password' /var/log/secure | grep -E -o '(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)' | uniq -c")
                print(output)

            elif choice == '13':
                output, _ = self.execute_command("""sudo grep 'Failed password' /var/log/secure | perl -e 'while($_=<>){ /for(.*?) from/; print "$1\\n";}' | uniq -c | sort -nr""")
                print(output)

            elif choice == '14':
                output, _ = self.execute_command("sudo grep 'sshd.*Accepted' /var/log/secure | awk '{print $1,$2,$3,$9,$11}'")
                print(output)
